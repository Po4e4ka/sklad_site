...
from .sklad_exceptions import DataTypeError, PeriodTypeError, IntTypeError
...
class MiddlewareInt:
    ...
    def process_exception(self, reqest, exception):
        if isinstance(exception, IntTypeError):
            # print("response from Middlewares")
            return JsonResponse({"DataTypeError": "object is not int"}, status=404)
