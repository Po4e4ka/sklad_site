...
def data_test(data):
    if type(data) != dict:
        logging.info("Wrong type of data")
...
def vvod_info_pos(data: dict):
    ...
    if "name" in data:
        if Positions.objects.filter(name=data["name"]).exists():
            logging.info(f"Есть в базе {data['name']}")
            return HttpResponse("Bad Request: positions.name is already in base")
        else:
            try:
                position = Positions(**data)
                position.save()
                logging.info(f"Сохранено в базе: {position.id}, {position.name}, {position.quantity}")
    ...
    if "name" in data:
        if Groups.objects.filter(name=data["name"]).exists():
            ...
            try:
                group = Groups(**data)
                group.save()
                logging.info(f"Сохранено в базе: {group.id}, {group.name}")
...
def vvod_info_xyz(data: dict):
    ...
    if "X" and "y" not in data:
        logging.info("Координаты не определены, введите параметры: x - латинские буквы от A до Z, y - число от 1 до 9")
        ...
        try:
            if "z" not in data and Xyz.objects.filter(X=data["X"], y=data["y"]).exists():
                logging.info(f"Есть в базе {data['X']}, {data['y']}")
                ...
                logging.info(f"Есть в базе {data['X']}, {data['y']}, {data['z']}")
                return HttpResponse("Bad Request: Xyz.name is already in base")
            else:
                xyz = Xyz(**data)
                xyz.save()
                logging.info(f"Сохранено в базе: {xyz.id}, {xyz.X}, {xyz.y}, {xyz.z}")
    ...
    if "name" in data:
        if Levels.objects.filter(name=data["name"]).exists():
            ...
            try:
                level = Levels(**data)
                level.save()
                logging.info(f"Сохранено в базе: {level.id}, {level.name}")
...
def vvod_info_person(data: dict):
    ...
    if "name" in data:
        if Persons.objects.filter(name=data["name"]).exists():
            logging.info("Есть в базе", data["name"])
            return HttpResponse("Bad Request: persons.name is already in base")
        else:
            try:
                person = Persons(**data)
                person.save()
                logging.info(f"Сохранено в базе: {person.id}, {person.name}")
    ...
    if "name" in data:
        if Objects.objects.filter(name=data["name"]).exists():
            ...
            try:
                obj = Objects(**data)
                obj.save()
                logging.info(f"Сохранено в базе: {obj.id}, {obj.name}")
    ...
    if "name" in data:
        if Change_types.objects.filter(name=data["name"]).exists():
            ...
            try:
                ...
                ch_type.save()
                logging.info(f"Сохранено в базе: {ch_type.id}, {ch_type.name}")
...
def vvod_info_ch_qant(data: dict = qant_test, type=type_test):
    ...
    if type < 5:
        if not Positions.objects.filter(id=data["position_id"]).exists():
            logging.info("Позиция не найдена", data["position_id"])
        ...
        pos.save()
        logging.info(f"Сохранено в базе: {pos.id},{pos.name}, {pos.quantity}")
